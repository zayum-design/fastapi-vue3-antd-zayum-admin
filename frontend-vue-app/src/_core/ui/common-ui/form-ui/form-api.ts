...
import type { Recordable } from '@/_core/typings';

import type { FormActions, FormSchema, ZayumFormProps } from './types';

import { toRaw } from 'vue';

import { Store } from '@/_core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
  ...
  StateHandler,
} from '@/_core/shared/utils';

function getDefaultState(): ZayumFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<ZayumFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | ZayumFormProps = null;
  stateHandler: StateHandler;

  public store: Store<ZayumFormProps>;

  // 最后一次点击提交时的表单值
  private latestSubmissionValues: null | Recordable<any> = null;

  private prevState: null | ZayumFormProps = null;

  constructor(options: ZayumFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<ZayumFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: ZayumFormProps) => Partial<ZayumFormProps>)
      | Partial<ZayumFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<ZayumForm /> is not mounted');
    }
    return this.form;
  }

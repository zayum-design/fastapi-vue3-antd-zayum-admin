...
import type { AnyFunction } from '@/_core/types';
...
import { useHoverToggle } from '@/_core/hooks';
import { LockKeyhole, LogOut } from '@/_core/ui/icons';
...
import { LockScreenModal } from '@/layouts/widgets/lock-screen';
...
const [LockModal, lockModalApi] = useZayumModal({
  connectedComponent: LockScreenModal,
});
const [LogoutModal, logoutModalApi] = useZayumModal({
  onConfirm() {
    handleSubmitLogout();
  },
});
    ...
    <DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
      <div class="hover:bg-accent ml-1 mr-2 cursor-pointer rounded-full p-1.5">
        <div class="hover:text-accent-foreground flex-center">
          <ZayumAvatar :alt="text" :src="webURL+avatar" class="size-8" dot />
        </div>
      </div>
    </DropdownMenuTrigger>
      ...
      <div ref="refContent">
        <DropdownMenuLabel class="flex items-center p-3">
          ...
          <div class="ml-2 w-full">
            ...
            <div v-if="userInfo" class="mt-2 text-xs">
        ...
        <DropdownMenuItem
          ...
          @click="menu.handler"
        >
          <ZayumIcon :icon="menu.icon" class="mr-2 size-4" />
          {{ menu.text }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem

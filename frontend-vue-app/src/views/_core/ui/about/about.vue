...
import { ZayumRenderContent } from '@/_core/ui/common-ui/shadcn-ui';

import { Page } from '@/_core/ui/common-ui/components';
...
declare global {
  const __ZAYUM_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'zayum-link' },
    { default: () => text },
  );
...
const zayumDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(ZAYUM_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(ZAYUM_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(ZAYUM_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="ZAYUM_GITHUB_URL" class="zayum-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in zayumDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <ZayumRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>

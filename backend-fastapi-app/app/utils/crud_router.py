...
class CRUDRouter(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def __init__(
        ...
        self.router = APIRouter(
            prefix=prefix, tags=list(tags), dependencies=[Depends(get_current_admin)]
        )
    ...
    def _register_routes(self):
        ...
        self.router.add_api_route("/create", self.create_item, methods=["POST"])
        self.router.add_api_route("/update/{id}", self.update_item, methods=["PUT"])
        self.router.add_api_route("/delete/{id}", self.delete_item, methods=["DELETE"])
    ...
    def get_item(self, id: int, db: Session = Depends(get_db)):
        ...
        if db_obj is None:
            raise HTTPException(
                status_code=ErrorCode.NOT_FOUND.value, detail=f"{self.resource_name} not found."
            )

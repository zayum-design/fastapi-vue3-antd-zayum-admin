...
from app.modules.admin.sys_user_balance_log.schemas.sys_user_balance_log import (
    SysUserBalanceLogCreate,
    SysUserBalanceLogUpdate,
)
...
class CRUDSysUserBalanceLog:
    ...
    def get(self, db: Session, id: int) -> SysUserBalanceLog | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysUserBalanceLog, field):
                logger.error(_("Invalid sort field: {field} for model SysUserBalanceLog"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysUserBalanceLog"))
            return query
    ...
    def create(self, db: Session, obj_in: SysUserBalanceLogCreate) -> SysUserBalanceLog:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to create SysUserBalanceLog", exc_info=True)
            raise
    ...
    def update(
        ...
        obj_in: dict[str, Any] | SysUserBalanceLogUpdate,
    ) -> SysUserBalanceLog:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to update SysUserBalanceLog ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysUserBalanceLog | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysUserBalanceLog (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysUserBalanceLog:
    ...
    def first(self) -> SysUserBalanceLog | None:

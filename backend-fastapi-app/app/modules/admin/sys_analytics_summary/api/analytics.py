...
from app.core.cache import delete_cached_pattern_sync, get_cached_data_sync, set_cached_data_sync
from app.core.security import get_current_admin
from app.dependencies.database import get_db
from app.modules.admin.sys_admin_log.models.sys_admin_log import SysAdminLog
from app.modules.admin.sys_analytics_summary.crud.sys_analytics_summary import (
    crud_sys_analytics_summary,
)
from app.modules.admin.sys_analytics_summary.schemas.sys_analytics_summary import (
    SysAnalyticsSummaryCreate,
)
...
def clear_analytics_cache_sync():
    ...
    try:
        ...
        logger.error("AnalyticsCache: Failed to clear cache synchronously: {e}")


def query_ip_location(ip_address: str) -> str | None:
    """
    ...
    try:
        ...
        params = {"query": ip_address, "co": "", "resource_id": "6006", "oe": "utf8"}
        ...
        with httpx.Client(timeout=5.0) as client:
            ...
            if data.get("status") == "0" and data.get("data"):
                ...
                logger.warning("IP查询API返回的位置信息格式异常: {location_info}")
            else:
                logger.warning("IP查询API返回错误状态: {data.get('status')}")
            return None

    except httpx.RequestError:
        logger.error("IP查询请求失败: {e}")
    except httpx.HTTPStatusError:
        logger.error("IP查询HTTP错误: {e}")
    except Exception:
        logger.error("IP查询发生未知错误: {e}")
...
def get_ip_location_cache_key(ip: str) -> str | None:
    """从缓存获取IP地理位置"""
...
def save_analytics_summary_to_db(
    db: Session, summary_type: str, data: dict, summary_date: datetime | None = None
):
    """
    ...
    try:
        ...
        logger.info(
            "AnalyticsSummary: Saved {summary_type} summary to database with ID: {summary_id}"
        )

    except Exception:
        logger.error("AnalyticsSummary: Failed to save {summary_type} summary to database: {e}")
...
def get_analytics_overview(db: Session = Depends(get_db)):
    ...
    try:
        ...
        today_registered_users = (
            db.query(SysUser).filter(func.date(SysUser.created_at) == today).count()
        )

        today_logged_in_users = (
            db.query(SysUser).filter(func.date(SysUser.login_time) == today).count()
        )

        active_users_last_7_days = (
            db.query(SysUser).filter(SysUser.login_time >= seven_days_ago).count()
        )
        ...
        raise HTTPException(status_code=500, detail=f"获取分析数据失败: {e!s}")


@router.get("/trends")
def get_analytics_trends(days: int = Query(30, ge=1, le=365), db: Session = Depends(get_db)):
    """
    获取用户注册和访问趋势数据
    ...
    try:
        ...
        user_trends_query = (
            db.query(
                func.date(SysUser.created_at).label("date"), func.count(SysUser.id).label("count")
            )
            .filter(and_(SysUser.created_at >= start_date, SysUser.created_at <= end_date))
        ...
        while current_date <= end_date:
            ...
            user_trends.append({"date": date_str, "count": count})
        ...
        visit_trends_query = (
            ...
            .filter(and_(SysAdminLog.created_at >= start_date, SysAdminLog.created_at <= end_date))
        ...
        while current_date <= end_date:
            ...
            visit_trends.append({"date": date_str, "count": count})
            current_date += timedelta(days=1)
        ...
        raise HTTPException(status_code=500, detail=f"获取趋势数据失败: {e!s}")


@router.get("/visits")
def get_analytics_visits(
    period: str = Query("month", regex="^(month|week|day)$"), db: Session = Depends(get_db)
):
    """
    获取系统访问统计数据
    按时间周期统计访问次数和操作类型分布
    ...
    try:
        ...
        for visit in visits_by_time:
            ...
            try:
                ...
                count_int = 0
            visits_data.append({"time": visit.time_group, "count": count_int})
            total_visits_count += count_int

        # 按操作类型查询访问数据
        visits_by_action = (
            db.query(SysAdminLog.title, func.count(SysAdminLog.id).label("count"))
            .filter(and_(SysAdminLog.created_at >= start_date, SysAdminLog.created_at <= end_date))
            .group_by(SysAdminLog.title)
            .all()
        )
        ...
        for action in visits_by_action:
            if action.title:  # 只包含有值的操作
                action_data.append({"action": action.title, "count": action.count})
        ...
        raise HTTPException(status_code=500, detail=f"获取访问数据失败: {e!s}")


@router.get("/sources")
def get_analytics_sources(db: Session = Depends(get_db)):
    """
    获取用户来源和操作来源统计数据
    ...
    try:
        # 查询用户来源（按平台）
        user_sources = (
            db.query(SysUser.platform, func.count(SysUser.id).label("count"))
            .group_by(SysUser.platform)
            .all()
        )
        ...
        for source in user_sources:
            ...
            source_data.append({"source": platform_name_cn, "count": source.count})
            platform_distribution[platform_name_cn] = source.count

        result = {"userSources": source_data}

        # 缓存结果
        cache_analytics_data_sync("sources", cache_params, result)

        # 将来源数据保存到数据库
        summary_data = {"user_group_distribution": platform_distribution}
        save_analytics_summary_to_db(db, "daily", summary_data, datetime.now())

        return success_response(result)

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取来源数据失败: {e!s}")


@router.get("/monthly-logins")
def get_monthly_login_stats(months: int = Query(12, ge=1, le=24), db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        for month_range in month_ranges:
            login_count = (
                db.query(func.count(func.distinct(SysUser.id)))
                .filter(
                    and_(
            ...
            monthly_logins.append({"month": month_range["month"], "count": login_count})

        cache_analytics_data_sync("monthly-logins", cache_params, monthly_logins)

        summary_data = {"total_logins": sum(login["count"] for login in monthly_logins)}
        save_analytics_summary_to_db(db, "monthly", summary_data, datetime.now())

        return success_response(monthly_logins)

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取月度登录数据失败: {e!s}")


@router.get("/regions")
def get_analytics_regions(db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        try:
            ...
            for i, region in enumerate(common_regions):
                ...
                region_data.append({"region": region, "count": region_count})

            logger.warning("使用模拟地区数据作为降级方案")
            return success_response(region_data)

        except Exception:
            raise HTTPException(status_code=500, detail=f"获取地区数据失败: {e!s}")

...
from app.modules.admin.sys_analytics_summary.models.sys_analytics_summary import SysAnalyticsSummary
from app.modules.admin.sys_analytics_summary.schemas.sys_analytics_summary import (
    SysAnalyticsSummaryCreate,
    SysAnalyticsSummaryUpdate,
)
...
class CRUDSysAnalyticsSummary:
    ...
    def get(self, db: Session, id: int) -> SysAnalyticsSummary | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysAnalyticsSummary, field):
                logger.error(_("Invalid sort field: {field} for model SysAnalyticsSummary"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysAnalyticsSummary"))
            return query
    ...
    def create(self, db: Session, obj_in: SysAnalyticsSummaryCreate) -> SysAnalyticsSummary:
        ...
        try:
            ...
            if hasattr(obj_in, "summary_type") and obj_in.summary_type is not None:
                filters.append(SysAnalyticsSummary.summary_type == obj_in.summary_type)
            else:
            ...
            if hasattr(obj_in, "summary_date") and obj_in.summary_date is not None:
                filters.append(SysAnalyticsSummary.summary_date == obj_in.summary_date)
            else:
                all_fields_present = False  # If any part of composite key is None, skip check or handle as per policy
                # Depending on DB, NULLs might not be considered equal for unique constraints.
                # For now, if any part is None, we assume it won't cause a unique violation by itself.
            ...
            if all_fields_present and len(filters) == 2:
                ...
                if existing:
                    values_str = ", ".join(
                        [f"{getattr(obj_in, c)}" for c in ["summary_type", "summary_date"]]
                    )
                    raise ValueError(
                        _(
                            "Duplicate combination of values for fields: summary_type, summary_date. Values: [{values_str}]"
                        )
                    )
            ...
            if hasattr(obj_in, "summary_type") and obj_in.summary_type is not None:
                ...
                # Skip this constraint check if any field is None
                continue_check = False
            if hasattr(obj_in, "summary_year") and obj_in.summary_year is not None:
                filters.append(SysAnalyticsSummary.summary_year == obj_in.summary_year)
            else:
                all_fields_present = False  # If any part of composite key is None, skip check or handle as per policy
                # Depending on DB, NULLs might not be considered equal for unique constraints.
                # For now, if any part is None, we assume it won't cause a unique violation by itself.
                ...
                # Skip this constraint check if any field is None
                continue_check = False
            if hasattr(obj_in, "summary_month") and obj_in.summary_month is not None:
                filters.append(SysAnalyticsSummary.summary_month == obj_in.summary_month)
            else:
                all_fields_present = False  # If any part of composite key is None, skip check or handle as per policy
                # Depending on DB, NULLs might not be considered equal for unique constraints.
                # For now, if any part is None, we assume it won't cause a unique violation by itself.
            ...
            if all_fields_present and len(filters) == 3:
                ...
                if existing:
                    values_str = ", ".join(
                        [
                            f"{getattr(obj_in, c)}"
                            for c in ["summary_type", "summary_year", "summary_month"]
                        ]
                    )
                    raise ValueError(
                        _(
                            "Duplicate combination of values for fields: summary_type, summary_year, summary_month. Values: [{values_str}]"
                        )
                    )
            ...
            if hasattr(obj_in, "summary_type") and obj_in.summary_type is not None:
                ...
                # Skip this constraint check if any field is None
                continue_check = False
            if hasattr(obj_in, "region_name") and obj_in.region_name is not None:
                filters.append(SysAnalyticsSummary.region_name == obj_in.region_name)
            else:
                all_fields_present = False  # If any part of composite key is None, skip check or handle as per policy
                # Depending on DB, NULLs might not be considered equal for unique constraints.
                # For now, if any part is None, we assume it won't cause a unique violation by itself.
            ...
            if all_fields_present and len(filters) == 2:
                ...
                if existing:
                    values_str = ", ".join(
                        [f"{getattr(obj_in, c)}" for c in ["summary_type", "region_name"]]
                    )
                    raise ValueError(
                        _(
                            "Duplicate combination of values for fields: summary_type, region_name. Values: [{values_str}]"
                        )
                    )
            ...
            db.rollback()
            logger.error("Failed to create SysAnalyticsSummary", exc_info=True)
            raise
    ...
    def update(
        ...
        obj_in: dict[str, Any] | SysAnalyticsSummaryUpdate,
    ) -> SysAnalyticsSummary:
        ...
        try:
            ...
            fields_to_check = {
                f: update_data.get(f, getattr(db_obj, f)) for f in ["summary_type", "summary_date"]
            }
            is_changing = any(
                update_data.get(f) is not None and update_data.get(f) != getattr(db_obj, f)
                for f in ["summary_type", "summary_date"]
            )
            all_parts_not_none = all(
                fields_to_check[f] is not None for f in ["summary_type", "summary_date"]
            )

            if is_changing and all_parts_not_none:
                filters = [SysAnalyticsSummary.id != db_obj.id]
                ...
                if existing:
                    values_str = ", ".join(
                        [f"{fields_to_check[c]}" for c in ["summary_type", "summary_date"]]
                    )
                    raise ValueError(
                        _(
                            "Duplicate combination of values for fields: summary_type, summary_date. Values: [{values_str}]"
            ...
            # and at least one of them is changing, and none are being set to None (unless allowed by DB).
            fields_to_check = {
                f: update_data.get(f, getattr(db_obj, f))
                for f in ["summary_type", "summary_year", "summary_month"]
            }
            is_changing = any(
                update_data.get(f) is not None and update_data.get(f) != getattr(db_obj, f)
                for f in ["summary_type", "summary_year", "summary_month"]
            ...
            if is_changing and all_parts_not_none:
                ...
                filters.append(
                    SysAnalyticsSummary.summary_month == fields_to_check["summary_month"]
                )
                existing = db.query(SysAnalyticsSummary).filter(and_(*filters)).first()
                if existing:
                    values_str = ", ".join(
            ...
            # and at least one of them is changing, and none are being set to None (unless allowed by DB).
            fields_to_check = {
                f: update_data.get(f, getattr(db_obj, f)) for f in ["summary_type", "region_name"]
            }
            is_changing = any(
                update_data.get(f) is not None and update_data.get(f) != getattr(db_obj, f)
                for f in ["summary_type", "region_name"]
            )
            all_parts_not_none = all(
                fields_to_check[f] is not None for f in ["summary_type", "region_name"]
            )
            ...
            if is_changing and all_parts_not_none:
                ...
                if existing:
                    values_str = ", ".join(
                        [f"{fields_to_check[c]}" for c in ["summary_type", "region_name"]]
                    )
                    raise ValueError(
                        _(
                            "Duplicate combination of values for fields: summary_type, region_name. Values: [{values_str}]"
            ...
            db.rollback()
            logger.error("Failed to update SysAnalyticsSummary ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysAnalyticsSummary | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysAnalyticsSummary (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysAnalyticsSummary:
    ...
    def first(self) -> SysAnalyticsSummary | None:

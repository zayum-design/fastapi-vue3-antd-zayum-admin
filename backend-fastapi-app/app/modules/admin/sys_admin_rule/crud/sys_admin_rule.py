...
class CRUDSysAdminRule:
    ...
    def get(self, db: Session, id: int) -> SysAdminRule | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysAdminRule, field):
                logger.error(_("Invalid sort field: {field} for model SysAdminRule"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysAdminRule"))
            return query
    ...
    def create(self, db: Session, obj_in: SysAdminRuleCreate) -> SysAdminRule:
        ...
        try:
            ...
            if hasattr(obj_in, "name") and obj_in.name is not None:
                existing = db.query(SysAdminRule).filter(SysAdminRule.name == obj_in.name).first()
                if existing:
            ...
            db.rollback()
            logger.error("Failed to create SysAdminRule", exc_info=True)
            raise

    def update(
        self, db: Session, db_obj: SysAdminRule, obj_in: dict[str, Any] | SysAdminRuleUpdate
    ) -> SysAdminRule:
        ...
        try:
            ...
            if "name" in update_data and update_data["name"] is not None:
                ...
                if new_name != db_obj.name:
                    existing = (
                        db.query(SysAdminRule)
                        .filter(SysAdminRule.name == new_name, SysAdminRule.id != db_obj.id)
                        .first()
                    )
                    if existing:
            ...
            db.rollback()
            logger.error("Failed to update SysAdminRule ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysAdminRule | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysAdminRule (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysAdminRule:
    ...
    def first(self) -> SysAdminRule | None:

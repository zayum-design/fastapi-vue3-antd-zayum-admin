...
from sqlalchemy import DATETIME, JSON, Enum, String, text
...
class SysAdminRule(TimestampMixin, Base):
    ...
    permission: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    menu_display_type: Mapped[str | None] = mapped_column(
        Menu_display_typeEnum, server_default=text("'addtabs'")
    )

...
class SysAttachment(TimestampMixin, Base):
    ...
    att_type: Mapped[str | None] = mapped_column(
        Att_typeEnum, server_default=text("'image'"), nullable=True
    )
    thumb: Mapped[str | None] = mapped_column(String(255), nullable=True)
    path_file: Mapped[str] = mapped_column(String(255))
    file_name: Mapped[str | None] = mapped_column(String(100), nullable=True)
    file_size: Mapped[int] = mapped_column()
    mimetype: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ext_param: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    general_attachment_col: Mapped[str | None] = mapped_column(String(45), nullable=True)

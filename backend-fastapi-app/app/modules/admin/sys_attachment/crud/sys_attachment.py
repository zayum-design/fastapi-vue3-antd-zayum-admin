...
class CRUDSysAttachment:
    ...
    def get(self, db: Session, id: int) -> SysAttachment | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysAttachment, field):
                logger.error(_("Invalid sort field: {field} for model SysAttachment"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysAttachment"))
            return query
    ...
    def create(self, db: Session, obj_in: SysAttachmentCreate) -> SysAttachment:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to create SysAttachment", exc_info=True)
            raise

    def update(
        self, db: Session, db_obj: SysAttachment, obj_in: dict[str, Any] | SysAttachmentUpdate
    ) -> SysAttachment:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to update SysAttachment ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysAttachment | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysAttachment (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysAttachment:
    ...
    def first(self) -> SysAttachment | None:

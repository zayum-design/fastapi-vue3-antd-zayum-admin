...
from sqlalchemy import DATETIME, TEXT, Enum, FetchedValue, Integer, String
...
class SysNotification(TimestampMixin, Base):
    ...
    receiver_type: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=FetchedValue()
    )
    ...
    def validate_receiver_type_length(self, key, value):
        ...
        if len(value) > 20:
            logger.error("Value too long for {key}: {value} (max 20 chars)")
            raise ValueError(_("Value too long (max 20 characters)"))
        return value
    ...
    ] = mapped_column(TypeEnum, nullable=False, server_default=FetchedValue())
    status: Mapped[Literal["unread", "read"]] = mapped_column(

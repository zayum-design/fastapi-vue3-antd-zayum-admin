...
from app.modules.admin.sys_user_score_log.schemas.sys_user_score_log import (
    SysUserScoreLogCreate,
    SysUserScoreLogUpdate,
)
...
class CRUDSysUserScoreLog:
    ...
    def get(self, db: Session, id: int) -> SysUserScoreLog | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysUserScoreLog, field):
                logger.error(_("Invalid sort field: {field} for model SysUserScoreLog"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysUserScoreLog"))
            return query
    ...
    def create(self, db: Session, obj_in: SysUserScoreLogCreate) -> SysUserScoreLog:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to create SysUserScoreLog", exc_info=True)
            raise

    def update(
        self, db: Session, db_obj: SysUserScoreLog, obj_in: dict[str, Any] | SysUserScoreLogUpdate
    ) -> SysUserScoreLog:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to update SysUserScoreLog ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysUserScoreLog | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysUserScoreLog (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysUserScoreLog:
    ...
    def first(self) -> SysUserScoreLog | None:

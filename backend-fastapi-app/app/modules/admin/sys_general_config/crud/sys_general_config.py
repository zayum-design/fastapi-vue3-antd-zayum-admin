...
class CRUDSysGeneralConfig:
    ...
    def get(self, db: Session, id: int) -> SysGeneralConfig | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysGeneralConfig, field):
                logger.error(_("Invalid sort field: {field} for model SysGeneralConfig"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysGeneralConfig"))
            return query
    ...
    def create(self, db: Session, obj_in: SysGeneralConfigCreate) -> SysGeneralConfig:
        ...
        try:
            ...
            if hasattr(obj_in, "name") and obj_in.name is not None:
                existing = (
                    db.query(SysGeneralConfig).filter(SysGeneralConfig.name == obj_in.name).first()
                )
                if existing:
            ...
            db.rollback()
            logger.error("Failed to create SysGeneralConfig", exc_info=True)
            raise

    def update(
        self, db: Session, db_obj: SysGeneralConfig, obj_in: dict[str, Any] | SysGeneralConfigUpdate
    ) -> SysGeneralConfig:
        ...
        try:
            ...
            if "name" in update_data and update_data["name"] is not None:
                ...
                if new_name != db_obj.name:
                    existing = (
                        db.query(SysGeneralConfig)
                        .filter(SysGeneralConfig.name == new_name, SysGeneralConfig.id != db_obj.id)
                        .first()
                    )
                    if existing:
            ...
            db.rollback()
            logger.error("Failed to update SysGeneralConfig ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysGeneralConfig | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysGeneralConfig (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysGeneralConfig:
    ...
    def first(self) -> SysGeneralConfig | None:

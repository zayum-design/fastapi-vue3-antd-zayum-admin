...
class CRUDSysGeneralCategory:
    ...
    def get(self, db: Session, id: int) -> SysGeneralCategory | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysGeneralCategory, field):
                logger.error(_("Invalid sort field: {field} for model SysGeneralCategory"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysGeneralCategory"))
            return query
    ...
    def create(self, db: Session, obj_in: SysGeneralCategoryCreate) -> SysGeneralCategory:
        ...
        try:
            ...
            if hasattr(obj_in, "name") and obj_in.name is not None:
                existing = (
                    db.query(SysGeneralCategory)
                    .filter(SysGeneralCategory.name == obj_in.name)
                    .first()
                )
                if existing:
            ...
            db.rollback()
            logger.error("Failed to create SysGeneralCategory", exc_info=True)
            raise
    ...
    def update(
        ...
        obj_in: dict[str, Any] | SysGeneralCategoryUpdate,
    ) -> SysGeneralCategory:
        ...
        try:
            ...
            if "name" in update_data and update_data["name"] is not None:
                ...
                if new_name != db_obj.name:
                    existing = (
                        db.query(SysGeneralCategory)
                        .filter(
                            SysGeneralCategory.name == new_name, SysGeneralCategory.id != db_obj.id
                        )
                        .first()
                    )
                    if existing:
            ...
            db.rollback()
            logger.error("Failed to update SysGeneralCategory ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysGeneralCategory | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysGeneralCategory (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysGeneralCategory:
    ...
    def first(self) -> SysGeneralCategory | None:

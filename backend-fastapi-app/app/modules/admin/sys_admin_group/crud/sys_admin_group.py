...
from app.modules.admin.sys_admin_group.schemas.sys_admin_group import (
    SysAdminGroupCreate,
    SysAdminGroupUpdate,
)
...
class CRUDSysAdminGroup:
    ...
    def get(self, db: Session, id: int) -> SysAdminGroup | None:
    ...
    def _apply_order_by(self, query: Query, orderby: str | None) -> Query:
        ...
        try:
            ...
            if not hasattr(SysAdminGroup, field):
                logger.error(_("Invalid sort field: {field} for model SysAdminGroup"))
                return query
            ...
            logger.error(_("Sort field does not exist on model SysAdminGroup"))
            return query
    ...
    def create(self, db: Session, obj_in: SysAdminGroupCreate) -> SysAdminGroup:
        ...
        try:
            ...
            if hasattr(obj_in, "name") and obj_in.name is not None:
                existing = db.query(SysAdminGroup).filter(SysAdminGroup.name == obj_in.name).first()
                if existing:
            ...
            db.rollback()
            logger.error("Failed to create SysAdminGroup", exc_info=True)
            raise

    def update(
        self, db: Session, db_obj: SysAdminGroup, obj_in: dict[str, Any] | SysAdminGroupUpdate
    ) -> SysAdminGroup:
        ...
        try:
            ...
            if "name" in update_data and update_data["name"] is not None:
                ...
                if new_name != db_obj.name:
                    existing = (
                        db.query(SysAdminGroup)
                        .filter(SysAdminGroup.name == new_name, SysAdminGroup.id != db_obj.id)
                        .first()
                    )
                    if existing:
            ...
            db.rollback()
            logger.error("Failed to update SysAdminGroup ({db_obj.id})", exc_info=True)
            raise

    def remove(self, db: Session, id: int) -> SysAdminGroup | None:
        ...
        try:
            ...
            db.rollback()
            logger.error("Failed to delete SysAdminGroup (ID: {id})", exc_info=True)
            raise
...
class QueryBuilderSysAdminGroup:
    ...
    def first(self) -> SysAdminGroup | None:

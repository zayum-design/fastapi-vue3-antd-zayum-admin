...
from app.modules.admin.auth.crud.sys_auth_user import crud_sys_auth_user
from app.modules.admin.sys_user.schemas.sys_user import SysUser, SysUserCreate
from app.modules.admin.sys_user_group.schemas.sys_user_group import SysUserGroup
from app.modules.admin.sys_user_rule.crud.sys_user_rule import crud_sys_user_rule
...
async def login(
    ...
    if not user or not user.check_password(login_data.password):
        if user:
            ...
            user.login_time = datetime.now(UTC)
    ...
    logger.info(f"User {login_data.username} logged in successfully from IP: {client_ip}")
...
async def login_form(
    ...
    logger.info(f"User {form_data.username} logged in successfully from IP: {client_ip}")

    return {"access_token": access_token, "token_type": "bearer"}


@router.post("/sms_login", response_model=TokenResponse)
async def sms_login(login_data: SmsLoginInput, request: Request, db: Session = Depends(get_db)):
    # 模拟验证码验证
    ...
    return success_response(
        {
            ...
            "user_info": {"username": "test_user", "phone": "13800000000"},
        }
    )
...
async def qr_login(login_data: QrLoginInput, request: Request, db: Session = Depends(get_db)):
    # 模拟二维码验证
...
async def get_qr_code():
    # 模拟生成二维码
    return success_response({"qr_code": "test_qr_code", "expire_time": 0})


@router.post("/register", response_model=TokenResponse)
async def register(register_data: RegisterInput, db: Session = Depends(get_db)):
    # 检查用户名是否已存在
    ...
    user_data = SysUserCreate(
        ...
        else f"{register_data.username}@example.com",
...
async def forgot_password(forgot_data: ForgotPasswordInput, db: Session = Depends(get_db)):
    # 模拟验证码验证
...
async def social_login(
    login_data: SocialLoginInput, request: Request, db: Session = Depends(get_db)
):
    # 模拟第三方登录验证
    if login_data.code != "social_code":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="授权失败")
...
async def get_all_router(
    ...
    group = db.query(SysUserGroup).filter(SysUserGroup.id == current_user.user_group_id).first()

...
from app.schemas.sys_notification import SysNotificationCreate, SysNotificationUpdate
...
class CRUDSysNotification:
    ...
    def filter(self, db: Session, *criterion) -> 'QueryBuilderSysNotification':
        ...
        Args:
            ...
            *criterion: One or more SQLAlchemy filter expressions
                        (e.g., SysNotification.name == "example", SysNotification.status == 1).
        """
        initial_query = db.query(SysNotification)
        if criterion:
    ...
    def create(self, db: Session, obj_in: SysNotificationCreate) -> SysNotification:
        """Create new SysNotification record with uniqueness validation"""
        try:
            db_obj = SysNotification(**obj_in.model_dump(exclude_unset=True))
            db.add(db_obj)
            db.commit()
            db.refresh(db_obj)
            return db_obj
...
class QueryBuilderSysNotification:
    def __init__(self, db: Session, query: Query, crud_base: CRUDSysNotification):
        self._db: Session = db
...
crud_sys_notification = CRUDSysNotification()

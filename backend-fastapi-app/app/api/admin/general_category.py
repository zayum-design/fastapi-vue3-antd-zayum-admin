...
from app.models.sys_general_category import SysGeneralCategory as SysGeneralCategoryModel

# Initialize the API router for sys_general_category endpoints
router = APIRouter(
    prefix="/general/category", tags=["general_category"], dependencies=[Depends(get_current_admin)]
)
...
def read_sys_general_category(id: int, db: Session = Depends(get_db)):
    """
    Retrieve a single SysGeneralCategory record by its unique ID.

    Args:
        id (int): The unique identifier of the SysGeneralCategory.
        db (Session): Database session dependency.
...
def create_sys_general_category(obj_in: SysGeneralCategoryCreate, db: Session = Depends(get_db)):
    """
    Create a new SysGeneralCategory record.

    Args:
        obj_in (SysGeneralCategoryCreate): The schema containing the record's creation data.
        db (Session): Database session dependency.
...
def update_sys_general_category(id: int, obj_in: SysGeneralCategoryUpdate, db: Session = Depends(get_db)):
    ...
    Args:
        id (int): The unique identifier of the SysGeneralCategory to update.
        obj_in (SysGeneralCategoryUpdate): The schema containing the updated data.
        db (Session): Database session dependency.
...
def delete_sys_general_category(id: int, db: Session = Depends(get_db)):
    """
    Delete a SysGeneralCategory record by its unique ID.

    Args:
        id (int): The unique identifier of the SysGeneralCategory to delete.
        db (Session): Database session dependency.

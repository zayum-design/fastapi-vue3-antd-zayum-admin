...
from app.modules.admin.sys_user_balance_log.crud.sys_user_balance_log import (
    crud_sys_user_balance_log,
)
from app.modules.admin.sys_user.models.sys_user import SysUser
from app.utils.response_handlers import ErrorCode
from app.utils.responses import success_response

# Initialize the API router for user balance log endpoints
router = APIRouter(
    prefix="/balance/log", tags=["user_balance_log"], dependencies=[Depends(get_current_user)]
)
...
@router.get("/list")
def read_user_balance_log_list(
    page: int = 1,
    per_page: int = 10,
    ...
    """
    Retrieve a list of current user's balance log records with optional pagination, search, and sorting.
    ...
    base_query = db.query(SysUserBalanceLog).filter(SysUserBalanceLog.user_id == current_user.id)

    # Retrieve paginated records with search and sorting
    items = crud_sys_user_balance_log.get_multi(
        db, page=page, per_page=per_page, search=search, orderby=orderby, base_query=base_query
    )
    total = crud_sys_user_balance_log.get_total(db, search=search, base_query=base_query)
...
@router.get("/{id}")
def read_user_balance_log(
    id: int, 
    db: Session = Depends(get_db),
    current_user: SysUser = Depends(get_current_user),
):
    """
    Retrieve a single balance log record by its unique ID.
    Only returns the record if it belongs to the current user.

    Args:
        id (int): The unique identifier of the balance log.
        db (Session): Database session dependency.
        current_user (SysUser): Current logged-in user.
    ...
    if db_obj is None:
        raise HTTPException(
            status_code=ErrorCode.NOT_FOUND.value, detail=_("Balance log not found.")
        )

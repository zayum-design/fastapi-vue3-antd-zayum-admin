...
router = APIRouter(prefix="/plugin_store", tags=["plugin_store"])
...
async def read_sys_plugin_list(
    ...
    if type in ["all", "local"]:
        try:
            ...
            logger.info("正在扫描插件目录: {plugins_dir}")
            ...
            if not plugins_dir.exists():
                logger.error("插件目录不存在: {plugins_dir}")
            else:
                for plugin_path in plugins_dir.iterdir():
                    if plugin_path.is_dir():
                        plugin_dirs.append(plugin_path)
                        logger.info("发现插件目录: {plugin_path.name}")

                logger.info("总共发现 {len(plugin_dirs)} 个插件目录")

                for plugin_path in plugin_dirs:
                    try:
                        await plugin_loader._load_plugin(plugin_path, app)
                        logger.info("成功加载插件: {plugin_path.name}")
                    except Exception:
                        logger.error("加载插件 {plugin_path.name} 失败: {str(e)}")

            local_plugins = plugin_loader.list_plugins()
            logger.info("plugin_loader.list_plugins() 返回 {len(local_plugins)} 个插件")
                    ...
                    if plugin_json_path.exists():
                        try:
                            with open(plugin_json_path, encoding="utf-8") as f:
                                ...
                                logger.info(
                                    "直接从 {plugin_json_path} 加载插件: {plugin.get('name', '未知')}"
                                )
                        except Exception:
                            logger.error("读取插件JSON文件失败 {plugin_json_path}: {str(e)}")

            # Convert local plugins to match store plugin format
            for plugin in local_plugins:
                logger.info("处理本地插件: {plugin.get('name', '未知')}")
                # Check if plugin already exists in database by uuid
                existing_plugin = None
                if "uuid" in plugin:
                    existing_plugin = (
                        db.query(SysPluginModel)
                        .filter(SysPluginModel.uuid == plugin["uuid"])
                        .first()
                    )
                    if existing_plugin:
                        logger.info("插件 {plugin['uuid']} 在数据库中存在")
                    else:
                        logger.info("插件 {plugin['uuid']} 在数据库中不存在")
                ...
                plugin_item = {
                    ...
                    "installed": plugin.get(
                        "installed", 1 if existing_plugin else 0
                    ),  # 使用JSON中的installed值
            ...
            logger.info("本地插件处理完成，共 {local_total} 个插件")
        except Exception:
            logger.error("获取本地插件列表失败: {str(e)}", exc_info=True)
    ...
    if type == "all":
        ...
        for item in local_items:
            ...
            if uuid and uuid in plugin_map:
                ...
                logger.info("插件 {uuid} 使用本地数据覆盖数据库数据")
            elif uuid:
                # 如果uuid不存在，添加新的本地插件
                item["is_local"] = True
                plugin_map[uuid] = item
        ...
        if search:
            ...
            all_items = [
                ...
                if (
                    item.get("title", "").lower().find(search_lower) >= 0
                    or item.get("description", "").lower().find(search_lower) >= 0
                    or item.get("author", "").lower().find(search_lower) >= 0
                )
            ]
        ...
        return success_response(
            {
                "items": [{**item.to_dict(), "is_local": False} for item in store_items],
                "total": store_total,
                "page": page,
                "per_page": per_page,
            }

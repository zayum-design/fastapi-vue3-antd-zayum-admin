...
def read_sys_user_balance_log(id: int, db: Session = Depends(get_db)):
    ...
    if db_obj is None:
        ...
        raise HTTPException(
            status_code=ErrorCode.NOT_FOUND.value, detail=_("SysUserBalanceLog not found.")
        )
...
def update_sys_user_balance_log(
    id: int, obj_in: SysUserBalanceLogUpdate, db: Session = Depends(get_db)
):
    """

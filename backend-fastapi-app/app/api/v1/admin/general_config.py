...
def read_sys_general_config_list(
    ...
    items = crud_sys_general_config.get_multi(
...
def read_sys_general_config(id: int, db: Session = Depends(get_db)):
    ...
    if db_obj is None:
        ...
        raise HTTPException(
            status_code=ErrorCode.NOT_FOUND.value, detail=_("SysGeneralConfig not found.")
        )
...
def update_sys_general_config(
    id: int, obj_in: SysGeneralConfigUpdate, db: Session = Depends(get_db)
):
    """
...
@router.post("/save")
def save_general_config(request_data: dict, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        logger.info("Received save request with {len(request_data)} items")
        ...
        for key, value in request_data.items():
            ...
            try:
                ...
                if index_end == -1:
                    logger.warning("Invalid key format (missing ']' for index): {key}")
                    continue

                index_str = remaining[:index_end]
                remaining = remaining[index_end + 1 :]  # Skip the "]"
                logger.debug("Extracted index_str: {index_str}, remaining after index: {remaining}")

                # Check if we have [field] pattern
                if not remaining.startswith("["):
                    logger.warning("Invalid key format (missing '[' for field): {key}")
                    continue
                ...
                if not remaining.endswith("]"):
                    logger.warning("Invalid key format (missing trailing ']'): {key}")
                    continue

                field_type = remaining[:-1]  # Remove trailing "]"
                logger.debug("Extracted field_type: {field_type}")

                # Parse index
                try:
                    idx = int(index_str)
                except ValueError:
                    logger.warning("Invalid index (not a number): {index_str} in key: {key}")
                    idx = 0

                logger.debug(
                    "Final parsed values: config_name={config_name}, idx={idx}, field_type={field_type}, value={value}"
                )
                ...
                if not isinstance(updates[config_name], list):
                    ...
                    logger.warning("Config {config_name} was simple value, converting to array")
                    updates[config_name] = []
                ...
                if field_type == "key":
                    ...
                    logger.debug("Array config key: {config_name}[{idx}].key = {value}")
                elif field_type == "value":
                    updates[config_name][idx]["value"] = value
                    logger.debug("Array config value: {config_name}[{idx}].value = {value}")
                else:
                    logger.warning("Unknown field type: {field_type} for key: {key}")

            except Exception:
                logger.error("Error parsing key {key}: {str(e)}")
                continue

        logger.info("Parsed updates for {len(updates)} config items")
        # 记录所有解析的更新
        for config_name, new_value in updates.items():
            if isinstance(new_value, list):
                logger.info(
                    "Config {config_name} is array with {len(new_value)} items: {new_value}"
                )
            else:
                logger.info("Config {config_name} is simple value: {new_value}")
        ...
        for config_name, new_value in updates.items():
            ...
            config_item = (
                db.query(SysGeneralConfig).filter(SysGeneralConfig.name == config_name).first()
            )
                ...
                if isinstance(new_value, list):
                    ...
                    for idx, item in enumerate(new_value):
                        ...
                        if key is not None and key != "":
                            ...
                            logger.debug(
                                "Processing item {idx} for {config_name}: key='{key}', value='{value}'"
                            )
                    ...
                    logger.info("Converted array to JSON for {config_name}: {new_value_str}")
                    logger.debug("Array items: {new_value}, Result dict: {value_dict}")
                else:
                    new_value_str = str(new_value) if new_value is not None else ""
                    logger.info("Simple value for {config_name}: {new_value_str}")

                # Update the value
                config_item.value = new_value_str
                db.add(config_item)
                updated_count += 1
                logger.info("Updated config {config_name} with value: {new_value_str}")
            else:
                logger.warning("Config item not found: {config_name}")

        # Commit all changes
        db.commit()
        logger.info("Successfully updated {updated_count} config items")
        ...
        db.rollback()
        logger = logging.getLogger(__name__)
        logger.error("Error saving general config: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=_("Failed to save configuration: {error}").format(error=str(e))
        )

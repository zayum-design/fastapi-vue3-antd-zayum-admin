...
from app.modules.admin.sys_notification.crud.sys_notification import crud_sys_notification
...
router = APIRouter(
    prefix="/notifications", tags=["notifications"], dependencies=[Depends(get_current_admin)]
)
...
def get_notifications_list(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取通知列表失败: {e!s}")
...
def mark_all_notifications_read(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"标记通知为已读失败: {e!s}")
...
def clear_all_notifications(db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("清空通知失败: {e}")
        raise HTTPException(status_code=500, detail=f"清空通知失败: {e!s}")

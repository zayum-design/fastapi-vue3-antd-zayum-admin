...
class PluginLoader:
    def __init__(self, plugin_dir: Path):
        logger.info("初始化插件加载器，插件目录: {plugin_dir}")
    ...
    async def install_from_url(self, url: str):
        ...
        temp_file = None
        try:
            logger.info("开始下载插件包: {url}")

            # # 1. 下载插件包
            # temp_file = await download_service.download_plugin(url)
            # logger.info("插件包下载完成: {temp_file}")
            ...
            # logger.info("开始安装插件: {plugin_id}")
            # return await self._install_from_file(temp_file, plugin_id)

        except Exception as e:
            logger.error("安装插件时发生错误: {str(e)}")
            raise HTTPException(status_code=500, detail=f"安装失败: {e!s}")

        finally:
            if temp_file:
                # await download_service.cleanup(temp_file)
                logger.info("临时文件清理完成: {temp_file}")

    async def _validate_plugin(self, zip_path: Path) -> bool:
        """验证插件包的有效性"""
        try:
            logger.info("正在验证插件包: {zip_path}")
            with zipfile.ZipFile(zip_path, "r") as zip_ref:
                ...
                if required_file not in zip_ref.namelist():
                    logger.warning("插件包缺少必要的文件: {required_file}")
                    return False
            return True
        except zipfile.BadZipFile:
            logger.error("插件包不是有效的ZIP文件: {zip_path}")
            return False
    ...
    def _extract_plugin_id(self, url: str) -> str:
        ...
        logger.info("从URL中提取插件ID: {plugin_id}")
        return plugin_id
    ...
    async def _install_from_file(self, zip_path: Path, plugin_id: str):
        ...
        logger.info("正在安装插件到: {extract_path}")
        ...
        if req_file.exists():
            ...
            try:
                ...
                logger.info("后端依赖安装成功: {result.stdout}")
            except subprocess.CalledProcessError as e:
                logger.error("后端依赖安装失败: {e.stderr}")
        ...
        if package_json.exists():
            ...
            try:
                ...
                logger.info("前端依赖安装成功: {result.stdout}")
            except subprocess.CalledProcessError as e:
                logger.error("前端依赖安装失败: {e.stderr}")
    ...
    def list_plugins(self):
        return [
            {**manifest, "routes": self._get_plugin_routes(plugin_id, manifest)}
    ...
    def _get_plugin_routes(self, plugin_id, manifest):
        ...
        for component_name in components:
            ...
            route = {
                ...
                "meta": {"title": f"{manifest.get('name', plugin_id)}{title}", "icon": icon},
            }
            routes.append(route)
    ...
    async def _load_plugin(self, plugin_path: Path, app: FastAPI):
        """加载插件主逻辑"""
        try:
            logger.info("正在加载插件: {plugin_path}")
            ...
            logger.info("正在加载插件: {plugin_id} ({manifest['version']})")
            ...
            logger.info("插件加载成功: {plugin_id}")
            return {"status": "active", "plugin": manifest}

        except HTTPException:
            raise
        except Exception as e:
            logger.error("插件加载失败: {str(e)}")
            # await self._cleanup_failed_plugin(plugin_path)
            raise HTTPException(500, f"插件加载失败: {e!s}") from e
    ...
    async def _load_backend(
        self, plugin_path: Path, backend_config: dict, app: FastAPI, plugin_id: str
    ):
        """加载后端模块"""
        ...
        if hasattr(plugin_module, "get_plugin_app"):
            ...
            logger.info("API路由注册成功: {api_prefix}")
    ...
    def _register_frontend_assets(self, plugin_path: Path, frontend_config: dict, app: FastAPI):
        ...
        if assets_dir.exists():
            app.mount(
                f"/plugins-assets/{frontend_config['entry']}", StaticFiles(directory=assets_dir)
            )
            logger.info("前端资源已挂载: {assets_dir}")
        ...
        if "module_federation" in frontend_config:
            ...
            if remote_entry.exists():
                ...
                logger.info("模块联邦入口文件已注册: {remote_entry}")
        ...
        if manifest_path.exists():
            ...
            logger.info("manifest.json访问路由已注册")
    ...
    async def _cleanup_failed_plugin(self, plugin_path: Path):
        ...
        try:
            if plugin_path.exists():
                ...
                logger.warning("已将失败插件移动到备份目录: {backup_path}")

                # 4. 记录详细日志
                logger.info(
                    f"插件备份详情: \n"
                    f"  - 原始路径: {plugin_path!s}\n"
                    f"  - 备份路径: {backup_path!s}\n"
                    f"  - 时间戳: {timestamp}"
                )
        except Exception as e:
            logger.error("插件清理失败: {str(e)}", exc_info=True)
            raise HTTPException(500, f"插件清理失败: {e!s}")

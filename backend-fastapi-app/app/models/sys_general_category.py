...
class SysGeneralCategory(TimestampMixin, Base):
    __tablename__ = 'sys_general_category'
    __table_args__ = ()
    ...
    @validates("type")
    def validate_type_length(self, key, value):
        if not value:
    ...
    thumb: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    ...
    def __repr__(self):
        return f'<SysGeneralCategory(id={self.id})>'

    @classmethod
    def from_dict(cls, data: dict) -> 'SysGeneralCategory':
        valid_keys = {'weigh', 'type', 'thumb', 'pid', 'status', 'description', 'id', 'keywords', 'name'}

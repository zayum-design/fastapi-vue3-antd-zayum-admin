...
const downloadCode = async () => {
  ...
  try {
    ...
    // 1. 创建服务目录
    const modulesDir = tableName;
    
    // 2. 创建子目录结构
    zip.file(`${modulesDir}/models/${tableName}.py`, modelCode.value);
    zip.file(`${modulesDir}/crud/${tableName}.py`, crudCode.value);
    zip.file(`${modulesDir}/schemas/${tableName}.py`, schemasCode.value);
    
    // 3. API 文件命名：移除 sys_ 前缀（如果存在）
    const apiFileName = tableName.replace(/^sys_/, "");
    zip.file(`${modulesDir}/api/${apiFileName}.py`, apiCode.value);
